#!/usr/bin/env python3
"""Neighbor-Chronos biology analysis: do Evo2 embedding neighbors reflect CRISPR dependency biology?

Same pipeline as the DEMETER2 analysis but using Chronos (CRISPRi) gene effect scores.
Cell line lineage annotations are borrowed from the DEMETER2 table.
Chronos cell lines not present in DEMETER2 are dropped.
...
def build_analysis_table(df: pl.DataFrame, emb_ids: set[str], dep_genes: set[str]) -> pl.DataFrame:
    ...
    df = df.with_columns(
        pl.col("gene_name").is_in(list(dep_genes)).alias("has_dep_gene")
    )
    before = df.height
    df = df.filter(pl.col("has_dep_gene"))
    log.info(f"  After {DATASET_LABEL} gene filter: {df.height:,} (dropped {before - df.height:,})")
...
# ── Stage 3: Chronos loading (with DEMETER2 lineage annotations) ──────

def load_chronos_with_lineage() -> tuple[dict[str, np.ndarray], np.ndarray, list[str]]:
    """Load Chronos scores, filter to DEMETER2-overlapping cell lines, borrow lineage."""
    log.info(f"Loading {DATASET_LABEL}...")
    ...
    log.info(f"  {DATASET_LABEL} raw: {len(gene_cols)} genes x {chron_df.height} cell lines")

    log.info("Loading DEMETER2 cell line annotations...")
    dem_df = pl.read_csv(str(DEMETER2_PATH))
    dem_meta = dem_df.select(["depmap_id", "lineage_1"])
    dem_ids = set(dem_meta["depmap_id"].to_list())

    chron_ids_before = chron_df.height
    chron_df = chron_df.filter(pl.col("depmap_id").is_in(list(dem_ids)))
    log.info(f"  {DATASET_LABEL} after DEMETER2 cell line filter: {chron_df.height} "
             f"(dropped {chron_ids_before - chron_df.height} cell lines)")

    chron_df = chron_df.join(dem_meta, on="depmap_id", how="left")

    cell_ids = chron_df["depmap_id"].to_list()
    ...
    log.info(f"  {DATASET_LABEL}: {len(gene_cols)} genes x {len(cell_ids)} cell lines")
    log.info(f"  {len(set(lineage_1))} unique lineages")
    return gene_to_vec, np.array(cell_ids), lineage_1


# ── Shared helpers (identical to DEMETER2 pipeline) ───────────────────
...
def make_figures(result_df: pl.DataFrame, csq_summary: pl.DataFrame) -> None:
    ...
    ax.set_xlabel(f"{DATASET_LABEL} profile Pearson correlation")
    ax.set_ylabel("Density")
    ax.set_title(f"Gene dependency profile similarity ({DATASET_LABEL}):\nEmbedding neighbors vs. random controls")
    ...
    ax.legend()
    fig.tight_layout()
    fig.savefig(FIG_DIR / "chronos_fig_profile_similarity_hist.png", dpi=150)
    plt.close(fig)
    log.info(f"  Saved chronos_fig_profile_similarity_hist.png")
    ...
    ax.set_ylabel(f"Δ mean profile correlation\n(neighbor − random)")
    ax.set_title(f"Effect size by consequence class ({DATASET_LABEL})")
    ax.axhline(0, color="black", linewidth=0.5)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "chronos_fig_consequence_delta.png", dpi=150)
    plt.close(fig)
    log.info(f"  Saved chronos_fig_consequence_delta.png")
    ...
    ax.set_ylabel(f"Mean {DATASET_LABEL} profile correlation")
    ax.set_title(f"Sanity check ({DATASET_LABEL}): same-gene vs. cross-gene vs. random")
    for i, v in enumerate(means):
        ax.text(i, v + 0.002, f"{v:.4f}", ha="center", fontsize=10)
    fig.tight_layout()
    fig.savefig(FIG_DIR / "chronos_fig_same_vs_cross_vs_random.png", dpi=150)
    plt.close(fig)
    log.info(f"  Saved chronos_fig_same_vs_cross_vs_random.png")
    ...
    if len(nb_lin) > 0 and len(rd_lin) > 0:
        ...
        ax.set_xlabel(f"Lineage-mean {DATASET_LABEL} profile Pearson correlation")
        ax.set_ylabel("Density")
        ax.set_title(f"Lineage dependency concordance ({DATASET_LABEL}):\nEmbedding neighbors vs. random controls")
        ax.axvline(np.mean(nb_lin), color="C0", linestyle="--", alpha=0.8)
        ax.axvline(np.mean(rd_lin), color="C1", linestyle="--", alpha=0.8)
        ax.legend()
        fig.tight_layout()
        fig.savefig(FIG_DIR / "chronos_fig_lineage_similarity_hist.png", dpi=150)
        plt.close(fig)
        log.info(f"  Saved chronos_fig_lineage_similarity_hist.png")
...
def write_readme(
    ...
    log.info("Writing chronos_README_results.md...")
    ...
    lines = [
        f"# Evo2 Embedding Neighbors vs. {DATASET_LABEL} (CRISPRi) Dependency Biology",
        "",
        "## 1. Question",
        "",
        f"**Primary:** Do cross-gene nearest neighbors in the Evo2 second-order embedding space "
        f"have more similar {DATASET_LABEL} dependency profiles than matched random cross-gene controls?",
        "",
        f"**Secondary:** Is this effect stronger for protein-altering coding variants than for synonymous variants?",
        "",
        "## 2. Cohort",
        "",
        f"- **Variants retained:** {analysis_df.height:,}",
        f"- **Unique genes:** {analysis_df['gene_name'].n_unique():,}",
        f"- **Cell lines (shared with DEMETER2):** {n_cell_lines}",
    ...
    same_mean_val = nb_same["profile_corr"].mean() if nb_same.height > 0 else None
    lines += [
        "",
        "## 6. Sanity Checks",
        "",
        ...
        "",
        f"- Same-gene neighbor mean corr: {same_mean_val:.4f}" if same_mean_val else "- Same-gene: N/A",
        ...
        "## 7. Caveats",
        "",
        f"- Cell line annotations (lineage) borrowed from DEMETER2; only the {n_cell_lines} "
        "overlapping cell lines are used.",
        "- Embeddings are variant-level but dependency labels are gene-level; "
        ...
        f"- {DATASET_LABEL} measures CRISPRi-based gene dependency (Chronos algorithm), "
        "which differs from RNAi-based DEMETER2 in both mechanism and computational model.",
        "- This is association, not mechanism.",
        "",
    ]

    readme_path = OUT_DIR / "chronos_README_results.md"
...
def main() -> None:
    ...
    save_run_config(OUT_DIR, {
        ...
        "data_paths": {
            "duckdb": str(DB_PATH),
            "chronos": str(CHRONOS_PATH),
            "demeter2_annotations": str(DEMETER2_PATH),
    ...
    # ── Stage 2 ──
    log.info("=" * 60)
    log.info(f"STAGE 2: Build analysis table ({DATASET_LABEL})")
    log.info("=" * 60)
    raw_df = load_variant_data()
    emb_ids = get_embedding_ids()
    gene_to_vec, cell_ids, lineage_list = load_chronos_with_lineage()
    dep_genes = set(gene_to_vec.keys())
    n_cell_lines = len(cell_ids)
    analysis_df = build_analysis_table(raw_df, emb_ids, dep_genes)
    analysis_df = analysis_df.sort("variant_id")
    analysis_df.write_parquet(OUT_DIR / "chronos_analysis_table.parquet")
    log.info(f"  Saved chronos_analysis_table.parquet ({analysis_df.height:,} rows)")

    gene_list = sorted(analysis_df["gene_name"].unique().to_list())
    (OUT_DIR / "chronos_gene_list.txt").write_text("\n".join(gene_list) + "\n")
    log.info(f"  Saved chronos_gene_list.txt ({len(gene_list)} genes)")

    # ── Stage 3 ──
    log.info("=" * 60)
    log.info(f"STAGE 3: {DATASET_LABEL} gene profile lookup ready")
    log.info("=" * 60)
    gene_lineage_vecs = build_lineage_vectors(gene_to_vec, lineage_list)

    # ── Stage 4 ──
    log.info("=" * 60)
    log.info(f"STAGE 4: Cross-gene neighbor vs matched random ({DATASET_LABEL})")
    log.info("=" * 60)
    ...
    result_df.write_parquet(OUT_DIR / "chronos_neighbor_vs_random_profile_similarity.parquet")
    log.info(f"  Saved chronos_neighbor_vs_random_profile_similarity.parquet ({result_df.height:,} rows)")
    ...
    # ── Stage 5 ──
    log.info("=" * 60)
    log.info(f"STAGE 5: Consequence-bin robustness ({DATASET_LABEL})")
    log.info("=" * 60)
    csq_summary = consequence_summary(result_df)
    csq_summary = csq_summary.sort("reported_bin")
    csq_summary.write_parquet(OUT_DIR / "chronos_consequence_summary.parquet")
    log.info(f"  Saved chronos_consequence_summary.parquet")
    for row in csq_summary.iter_rows(named=True):
        d_str = f"{row['delta']:.4f}" if row["delta"] is not None else "N/A"
    ...
    # ── Stage 6 ──
    log.info("=" * 60)
    log.info(f"STAGE 6: Sanity checks ({DATASET_LABEL})")
    log.info("=" * 60)
    ...
    # ── Stage 7 ──
    log.info("=" * 60)
    log.info(f"STAGE 7: Figures ({DATASET_LABEL})")
    log.info("=" * 60)
    make_figures(result_df, csq_summary)

    # ── Stage 8 ──
    log.info("=" * 60)
    log.info(f"STAGE 8: README ({DATASET_LABEL})")
    log.info("=" * 60)
    write_readme(analysis_df, result_df, csq_summary, gc_mean, gc_std, lineage_stats, n_cell_lines)
    ...
    print("\n" + "=" * 60)
    print(f"TERMINAL SUMMARY ({DATASET_LABEL})")
    print("=" * 60)
    print(f"  Cell lines (DEMETER2 overlap): {n_cell_lines}")
